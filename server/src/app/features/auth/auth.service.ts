...
export default class AuthService {
  ...
  public async register(dto: RegisterDto) {
    ...
    const registerJWT = jwt.sign({ userId: savedUser.id }, process.env.JWT_SECRET || 'OBSCURA_DEV_SECRET_KEY', {
      expiresIn: '1h',
    });
  ...
  public async passwordReset(dto: { email: string }) {
    ...
    const resetJWT = jwt.sign({ userId: user.id }, process.env.JWT_SECRET || 'OBSCURA_DEV_SECRET_KEY', {
      expiresIn: '1h',
    });

    await this.emailService.sendVerificationEmail(user.email, 'Password Reset Request', resetJWT, user.email);
  ...
  public async newPassword(dto: { temporaryToken: string; newPassword: string; confirmPassword: string }) {
    // Implementation for password reset logic
    ...
    const decoded: any = jwt.verify(temporaryToken, process.env.JWT_SECRET || 'OBSCURA_DEV_SECRET_KEY');
